...
\end{proposition}
\begin{proof}
    First, $K$ meets only finitely many open cells of $C$. Otherwise pick a
    point $x_i$ in each of infinitely many distinct cells met by $K$; the set
    $S = \{x_1, x_2, \dots\}$ meets each closed cell in a finite (hence closed)
    set, so $S$ is closed in $C$, and the same argument applied to every subset
    of $S$ shows $S$ is discrete. A closed discrete subset of the compact set
    $K$ is finite, a contradiction.

    Since $K$ meets finitely many cells, it suffices to show each cell lies in a
    finite subcomplex. This follows by induction on the dimension: the image of
    the attaching map of an $n$-cell is compact, lies in the $(n-1)$-skeleton,
    and hence (by induction) in a finite subcomplex; adjoining the cell gives a
    finite subcomplex containing it. A finite union of finite subcomplexes is a
    finite subcomplex, so $K$ lies in one.
\end{proof}
